  ...
  return (
    <BookingData>
      {(bookingsData, restaurant) => {
        ...
        const activeBookings = safeBookingsData.filter(booking =>
        ...
        return (
          <div className='booking--page'>
            ...
            {isActiveBooking ? (
              <>
                {activeTab === 'cancelled' ? (
                  <div className='cancelled--wrapper' style={{ position: 'relative' }}>
                    {activeBookings.length > 0 && (
                      <React.Fragment key={activeBookings[currentCancelledIndex].id}>
                        <StatusRestaurants activeTab={activeTab} bookingData={activeBookings[currentCancelledIndex]} />
                        <div className="wrapper-status-order">
                          <StatusOrder
                            selectedGuests={activeBookings[currentCancelledIndex].selectedGuests}
                            selectedDate={activeBookings[currentCancelledIndex].selectedDate}
                            selectedTime={activeBookings[currentCancelledIndex].selectedTime}
                          />
                        </div>
                        <CurrentBooking title='' bookingData={activeBookings[currentCancelledIndex]} />
                      </React.Fragment>
                    )}
                    {activeBookings.length > 1 && (
                      <>
                        <button
                          ...
                          onClick={() => setCurrentCancelledIndex(prev => (prev - 1 + activeBookings.length) % activeBookings.length)}
                          style={{ position: 'fixed', left: 10, top: '50%', transform: 'translateY(-50%)' }}
                        >
                          <div className='icon'>
                        ...
                        <button
                          className='btn--default'
                          onClick={() => setCurrentCancelledIndex(prev => (prev + 1) % activeBookings.length)}
                          style={{ position: 'fixed', right: 10, top: '50%', transform: 'translateY(-50%)' }}
                        >
                  ...
                  <>
                    ...
                    {activeTab === 'cancelled' && orders.length > 0 && (() => {
                      const filteredOrders = orders.filter(order =>
                        order.items[0].restaurantId === activeBookings[currentCancelledIndex]?.restaurantId
                      );
                      const totalPages = Math.ceil(filteredOrders.length / ordersPerPage);

                      // Reset page if it's out of bounds after filtering
                      if (currentCancelledPage >= totalPages && totalPages > 0) {
                      ...
                      return (
                        <div style={{ padding: '20px', textAlign: 'center' }}>
                          ...
                          {filteredOrders
                            .slice(currentCancelledPage * ordersPerPage, (currentCancelledPage + 1) * ordersPerPage)
                            .map((order) => (
                          ...
                          {filteredOrders.length > ordersPerPage && (
                            <div style={{ marginTop: '20px' }}>
                              <button
                                onClick={() => setCurrentCancelledPage(prev => Math.max(0, prev - 1))}
                              ...
                              <button
                                onClick={() => setCurrentCancelledPage(prev => Math.min(totalPages - 1, prev + 1))}
                                disabled={currentCancelledPage === totalPages - 1}
                              >
                                Следующие
                              </button>
                            </div>
                ...
                {!shouldShowOrders() && activeTab === 'current' && (
                  <section className="show-menu--wrapper">
                    <div className="show-menu">
                      ...
                      <div className="info">
                        <p>Вы можете заказать еду к столу и выбрать удобное для вас время подачи блюд</p>
                      </div>
              ...
              <div style={{ padding: '40px', textAlign: 'center', color: '#666' }}>
                <h3>Нет {activeTab === 'current' ? 'текущих' : activeTab === 'cancelled' ? 'отмененных' : 'завершенных'} заказов</h3>
